---
name: verilog-build
description: Build a Vivado project module-by-module with live progress, timing analysis, and prompt-driven fixes.
---
...
1. **Resolve the project.**
   - If `the user's supplied request` is a path to an existing `.xpr` file, use it directly.
...
3. **Stream live progress.** Loop calling `get_build_status` (its built-in `wait_seconds`
   ...
   - If the user asks to change something mid-build even though nothing has failed, tell
     them to run `the `verilog-modify` skill` (or just say what they want changed and you can call
     `pause_build` yourself) rather than editing the source file while Vivado still has the
     project open for the current module.
   - If `status` becomes `"paused"` (from a `the `verilog-modify` skill` pause elsewhere), report that
