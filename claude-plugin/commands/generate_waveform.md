---
description: Simulate a module (combinational or sequential) and display its waveform in Vivado
argument-hint: "[project-path-or-name] [optional: module to simulate / stimulus description]"
---

Arguments: `$ARGUMENTS` (project path/name, optionally followed by which module to
...
1. **Resolve the project** the same way `/verilog-build` does: if `$ARGUMENTS` gives a
...
2. **Figure out which module to simulate.** If not obvious from `$ARGUMENTS`, ask. Read
